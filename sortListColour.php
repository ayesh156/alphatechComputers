...
if ($category == 0 && $brand == 0 && $condition == 0) {

    $query .= " WHERE `colour_id`= '" . $search . "' ";
} else if ($category == 0 && $brand != 0 && $condition == 0) {

    $query .= " INNER JOIN `brand_has_model` ON product.brand_has_model_id = brand_has_model.id WHERE brand_has_model.brand_id = '" . $brand . "'";
    $query .= " AND `colour_id`= '" . $search . "' ";
} else if ($category == 0 && $brand == 0 && $condition != 0) {

    $query .= " WHERE `colour_id`= '" . $search . "' AND `condition_id`='" . $condition . "' ";
} else if ($category != 0 && $brand == 0 && $condition == 0) {

    $query .= " WHERE `colour_id`= '" . $search . "' AND `category_id`='" . $category . "' ";
} else if ($category != 0 && $brand != 0 && $condition == 0) {

    $query .= " INNER JOIN `brand_has_model` ON product.brand_has_model_id = brand_has_model.id WHERE brand_has_model.brand_id = '" . $brand . "'";
    $query .= " AND `colour_id`= '" . $search . "' AND `category_id`='" . $category . "' ";
} else if ($category == 0 && $brand != 0 && $condition != 0) {

    $query .= " INNER JOIN `brand_has_model` ON product.brand_has_model_id = brand_has_model.id WHERE brand_has_model.brand_id = '" . $brand . "'";
    $query .= " AND `colour_id`= '" . $search . "' AND `condition_id`='" . $condition . "' ";
} else if ($category != 0 && $brand == 0 && $condition != 0) {

    $query .= " WHERE `colour_id`= '" . $search . "' AND `category_id`='" . $category . "' AND `condition_id`='" . $condition . "' ";
} else if ($category != 0 && $brand != 0 && $condition != 0) {

    $query .= " INNER JOIN `brand_has_model` ON product.brand_has_model_id = brand_has_model.id WHERE brand_has_model.brand_id = '" . $brand . "'";
    $query .= " AND `colour_id`= '" . $search . "' AND `category_id`='" . $category . "' AND `condition_id`='" . $condition . "' ";
}
    ...
    <ul>
        <li>
            <a <?php if ($pageno <= 1) {
                ...
                ?> onclick="sortListColour('<?php echo $search; ?>','<?php echo ($pageno - 1); ?>');" <?php } ?>>
        ...
        ?>
                ...
                <li>
                    <a class="active" onclick="sortListColour('<?php echo $search; ?>','<?php echo ($v); ?>');"><?php echo $v; ?></a>
                </li>
            ...
            ?>
                ...
                <li>
                    <a onclick="sortListColour('<?php echo $search; ?>','<?php echo ($v); ?>');"><?php echo $v; ?></a>
                </li>
